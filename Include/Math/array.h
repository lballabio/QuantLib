...
namespace QuantLib {
	...
	namespace Math {
		...
		/*! This class implements the concept of vector as used in linear algebra.
			As such, it is <b>not</b> meant to be used as a container - <tt>std::vector</tt>
		...
		*/
		class Array {
		  public:
			...
			/*! \name Vector algebra
				...
				<tt>v += x</tt> and similar operation involving a scalar value are shortcuts for
				\f$ \forall i : v_i = v_i + x \f$

				<tt>v *= w</tt> and similar operation involving two vectors are shortcuts for
				\f$ \forall i : v_i = v_i \times w_i \f$
				
				This implementation was inspired by T. L. Veldhuizen, <i>Expression templates</i>,
				C++ Report, 7(5):26-31, June 1995 available at 
		...
		// Abs()
			...
			inline Array Abs(const Array& v) {
				Array result(v.size());
				std::transform(v.begin(),v.end(),result.begin(),std::ptr_fun(QL_FABS));
				return result;
			}
		...
		// Sqrt()
			...
			inline Array Sqrt(const Array& v) {
				Array result(v.size());
				std::transform(v.begin(),v.end(),result.begin(),std::ptr_fun(QL_SQRT));
				return result;
			}
		...
		// Log()
			...
			inline Array Log(const Array& v) {
				Array result(v.size());
				std::transform(v.begin(),v.end(),result.begin(),std::ptr_fun(QL_LOG));
				return result;
			}
		...
		// Exp()
			...
			inline Array Exp(const Array& v) {
				Array result(v.size());
				std::transform(v.begin(),v.end(),result.begin(),std::ptr_fun(QL_EXP));
				return result;
			}
