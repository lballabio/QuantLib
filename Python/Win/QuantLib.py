...
class DepositRate2:
    def __init__(self,*args,**kwargs):
        self.this = apply(QuantLibc.new_DepositRate2,args,kwargs)
        self.thisown = 1

    def __del__(self,QuantLibc=QuantLibc):
        if self.thisown == 1 :
            QuantLibc.delete_DepositRate2(self)
    def maturity(*args, **kwargs):
        val = apply(QuantLibc.DepositRate2_maturity,args, kwargs)
        if val: val = DatePtr(val) ; val.thisown = 1
        return val
    def dayCounter(*args, **kwargs):
        val = apply(QuantLibc.DepositRate2_dayCounter,args, kwargs)
        if val: val = DayCounterPtr(val) ; val.thisown = 1
        return val
    def __repr__(self):
        return "<C DepositRate2 instance at %s>" % (self.this,)
class DepositRate2Ptr(DepositRate2):
    def __init__(self,this):
        self.this = this
        self.thisown = 0
        self.__class__ = DepositRate2


DepositRate2.rate = new.instancemethod(QuantLibc.DepositRate2_rate, None, DepositRate2)
