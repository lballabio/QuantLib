...
def main():
   print "Testing QuantLib.BarrierOption ... "
   totCases = 0
   startTime = time.time()
   error_list = []
   numErrors = 0
   ...
   for inputPara in iv:
      ...
      for i in (0,1):
         totCases  = totCases + 1
         op = QuantLib.BarrierOption(barrType, optType, underPrice, strike,
               divRate, rRate, resTime, vol[i], barrier, rebate)
         val = op.value()
         error = math.fabs(val - results[i])
         if error >= maxErrorAllowed:
            print "%7s %5s" %(barrType, optType),
            print "%4i %4i" %(strike, barrier),
            print "%8.4f %8.4f %12.2e" % (val, results[i], error)
            numErrors = numErrors + 1
         error_list.append(error)
   if numErrors > 0:
      print "Test NOT passed:  maximum error = %9.1e," %(max(error_list)),
      print "%i/%i cases did not pass the test!!!" % (numErrors, totCases)
   else:
      print " ... Test passed: maximum error = %9.1e, " %(max(error_list)),
      print "time elapsed %10.3f" % (time.time()-startTime)
