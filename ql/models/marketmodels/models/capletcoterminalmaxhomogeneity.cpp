...
namespace
{
		...
		)
	{
		...
		Matrix orthogonalTransformationRestricted(basis.getBasisAsRowsInMatrix());
		Matrix orthogonalTransformation(capletNumber+1,capletNumber+1 , 0.0);

		orthogonalTransformation[capletNumber][capletNumber] =1.0;
		for (Size k=0; k < capletNumber+1; ++k)
			for (Size l=0; l < capletNumber+1; ++l)
				orthogonalTransformation[k][l] = orthogonalTransformationRestricted[k][l];

		Array movedCentre = orthogonalTransformationRestricted*cylinderCentre;
		Real alpha = movedCentre[0];
		Array movedTarget = orthogonalTransformation*targetArray;
		...
		Array arraySolution(transpose(orthogonalTransformation)*rotatedSolution);
    	...
    	for (Size i =0; i < numberOfRates-1; ++i) {
        	...
        	bool success=
					singleRateClosestPointFinder(
																		...
																		w0,
																		w1,
																		iterationsForMinimzation,
			                                                            toleranceForMinimization,
																		theseNewVols);
