...
namespace QuantLib {
		...
		)
	{
		...
		{ // we will calibrate caplet on forward rate i,
			...
			if (maximizeHomogeneity)
				success = solver.solveWithMaxHomogeneity(alphaInitial[i+1] ,
				i,
				rateonevols,
				ratetwovols,
				correlations,			
		...
		)
	{
		...
		do
		{
			...
			Matrix capletTotCovariance = flmm.totalCovariance(evolution.numberOfRates()-1);

			std::vector<Volatility> capletVolsOutput(numberOfRates);
			for (Size i=0; i<numberOfRates; ++i) {
				capletVolsOutput[i] = std::sqrt(capletTotCovariance[i][i]/evolution.rateTimes()[i]);
			}
			...
			for (Size i=0; i<numberOfRates-1; ++i) 
			{
				...
				modifiedCapletVols[i] = modifiedCapletVols[i]*capletVols[i]/capletVolsOutput[i];
			}
			++iterations;
		}
		while (iterations < maxIterations && sqrt(error) > toleranceForIterativeSolving);
