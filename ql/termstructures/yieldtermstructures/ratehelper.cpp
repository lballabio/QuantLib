...
namespace QuantLib {
    ...
    Real RateHelper::quoteValue() const {
        return quote_->value();
    }

    bool RateHelper::quoteIsValid() const {
        return quote_->isValid();
    }

    DiscountFactor RateHelper::discountGuess() const {
        return Null<Real>();
    }
    ...
    Date RateHelper::earliestDate() const {
        return earliestDate_;
    }

    Date RateHelper::latestDate() const {
        return latestDate_;
    }

    void RateHelper::update() {
        notifyObservers();
    }
