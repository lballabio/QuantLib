...
namespace QuantLib {
    ...
    */
    class ImpliedTermStructure : public DerivedTermStructure<> {
      public:
    ...
    : DerivedTermStructure(std::move(h), referenceDate) {}

    inline ImpliedTermStructure::ImpliedTermStructure(Handle<YieldTermStructure> h,
                                                      Natural settlementDays,
                                                      Calendar calendar)
    : DerivedTermStructure(std::move(h), settlementDays, std::move(calendar)) {}

    inline void ImpliedTermStructure::update() {
        refDf_ = Null<DiscountFactor>();
        refTime_ = Null<Time>();
        DerivedTermStructure::update();
    }
