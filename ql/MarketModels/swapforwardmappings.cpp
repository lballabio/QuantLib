...
namespace QuantLib {
    ...
    Disposable<Matrix> SwapForwardMappings::coterminalSwapForwardJacobian(
        const CurveState& cs) 
    {
    ...
    Disposable<Matrix> SwapForwardMappings::coterminalSwapZedMatrix(
        const CurveState& cs,
        Spread displacement) 
    {
    ...
    Disposable<Matrix> SwapForwardMappings::coinitialSwapForwardJacobian(
        const CurveState& cs) 
    {
        // FIXME
        //std::vector<Real> b = cs.coinitialSwapRatesAnnuities();
        std::vector<Real> b = cs.coterminalSwapRatesAnnuities();
        Size n = b.size();
        const std::vector<Real> p = cs.discountRatios();
        const std::vector<Rate> f = cs.forwardRates();
    ...
    Disposable<Matrix> SwapForwardMappings::coinitialSwapZedMatrix(
        const CurveState& cs,
        Spread displacement) 
    {
        Matrix result = coinitialSwapForwardJacobian(cs);
        const std::vector<Rate> f = cs.forwardRates();
        // FIXME
        //const std::vector<Rate> sr = cs.coinitialSwapRates();
        const std::vector<Rate> sr = cs.coterminalSwapRates();
        for (Size i=0; i<sr.size(); ++i) {
            for (Size j=i; j<f.size(); ++j) {
                result[i][j] *= (f[j]+displacement)/(sr[i]+displacement);
