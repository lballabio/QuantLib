...
*/
#include <ql/MarketModels/Evolvers/forwardrateconstrainedeuler.h>
...
namespace QuantLib {
	...
	void ForwardRateConstrainedEuler::SetThisConstraint(const std::vector<Rate>& rateConstraints,
	...
	// check constraint active
		if (isConstraintActive_[currentStep_])
		{
			...
			// now shift each rate by multiplier * weighting of index rate  across the step

			for (Size i=alive; i<n_; i++) {
					logForwards_[i] += multiplier*covariances_[currentStep_][i]; // we only need a small part of cov matrix
			}
