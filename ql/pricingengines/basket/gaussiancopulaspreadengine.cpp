...
namespace QuantLib {
    ...
    void GaussianCopulaSpreadEngine::calculate() const {
        ...
        for (Size i = 0; i < gh.order(); ++i) {
            ...
            const Real u1 = std::clamp(Phi(z1), static_cast<Real>(QL_EPSILON), static_cast<Real>(1.0 - QL_EPSILON));
            ...
            for (Size j = 0; j < gh.order(); ++j) {
                ...
                const Real u2 = std::clamp(Phi(z2), static_cast<Real>(QL_EPSILON), static_cast<Real>(1.0 - QL_EPSILON));
