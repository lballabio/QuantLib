...
namespace QuantLib {
    ...
    namespace detail {
        ...
        Real SumExponentialsRootSolver::getRoot(Real xTol, Strategy strategy) const {
            ...
            const Real denom = std::accumulate(attr.begin(), attr.end(), static_cast<Real>(0.0));
            const Real xInit = (std::abs(denom) > 1000*QL_EPSILON)
                ? std::min(10.0, std::max(-10.0,
                      (K_ - std::accumulate(a_.begin(), a_.end(), static_cast<Real>(0.0)))/denom)
                  )
    ...
    void SingleFactorBsmBasketEngine::calculate() const {
        ...
        if (std::all_of(
            ...
            results_.value = dr0*payoff->operator()(
                std::accumulate(fwdBasket.begin(), fwdBasket.end(), static_cast<Real>(0.0)));
        }
        else {
