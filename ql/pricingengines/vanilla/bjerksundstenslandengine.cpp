...
namespace QuantLib {
    ...
    BjerksundStenslandApproximationEngine::europeanCallResults(
        ...
        results.additionalResults["strikeGamma"] = static_cast<Real>(results.gamma*squared(S/X));
    ...
    BjerksundStenslandApproximationEngine::immediateExercise(Real S, Real X) const {
        ...
        results.additionalResults["strikeGamma"] = static_cast<Real>(0.0);
    ...
    BjerksundStenslandApproximationEngine::americanCallApproximation(
        ...
        if (S >= I) {
            ...
            const Real B0Dq = (dD <= rfD) ? static_cast<Real>(0.0)
                : static_cast<Real>(X*std::log(rfD)/squared(std::log(dD))*tq);
            ...
            const Real B0Dr = (dD <= rfD) ? static_cast<Real>(0) : static_cast<Real>(-X*tr/std::log(dD));
