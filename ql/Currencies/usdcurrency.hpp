...
namespace QuantLib {
	...
	public:
	    std::string name() const { return "U.S Dollar"; };
	    std::string mnemonic() const { return "USD"; };
	    std::string numeric() const { return "840"; };
	    std::string symbol() const { return "$"; };
	    bool isRounding() const { return false; }
	    Rounding prefRounding() const { return Rounding(); }
	    Currency nextCurrency() const { return USDCurrency(); }
	    Currency lastCurrency() const { return USDCurrency(); }
	};
    public:
	USDCurrency();
    };
