...
namespace QuantLib {
	...
	public:
        IndexedCashFlow(Real notional, const boost::shared_ptr<Index> &index,
                        const Date& baseDate, const Date& fixDate, const Date& payDate)
        : notional_(notional), index_(index), baseDate_(baseDate), fixDate_(fixDate), 
		  payDate_(payDate) {}
        //! \name Event interface
        //@{
        Date date() const { return payDate_; }
        //@}
		virtual Real notional() const { return notional_; }
		virtual Date baseDate() const { return baseDate_; }
		virtual Date fixDate() const { return fixDate_; }
	...
	private:
        ...
        Date baseDate_, fixDate_, payDate_;
    };
