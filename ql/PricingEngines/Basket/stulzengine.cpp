...
namespace QuantLib {
    ...
    namespace {
        ...
        double euroTwoAssetMinBasketCall(double s1_0, double s2_0, double strike, 
            DiscountFactor riskFreeDiscount, Rate intRate, double vol_s1, double vol_s2, 
            ...
            double sigma_squared = vol_s1 * vol_s1 + vol_s2 * vol_s2 - 2 * rho * vol_s1 * vol_s2;
        ...
        double euroTwoAssetMaxBasketCall(double s1_0, double s2_0, double strike, Rate riskFreeDiscount, 
            Rate intRate, double vol_s1, double vol_s2, double rho, Time expiry) {
            ...
            BlackFormula black1(forwardPrice1, riskFreeDiscount, vol_s1*vol_s1, payoff);
            BlackFormula black2(forwardPrice2, riskFreeDiscount, vol_s2*vol_s2, payoff);  

            return black1.value() + black2.value() - 
                        euroTwoAssetMinBasketCall(s1_0, s2_0, strike, 
                            riskFreeDiscount, intRate, vol_s1, vol_s2, rho, expiry);                       
        }
    }
    ...
    void StulzEngine::calculate() const {
        ...
        double vol_s1 = procs[0]->volTS->blackVol(exercise->lastDate(), strike);
        double vol_s2 = procs[1]->volTS->blackVol(exercise->lastDate(), strike);
        ...
        switch (basketType) {
          case BasketOption::Max:
            switch (payoff->optionType()) {
              ...
              case Option::Call:                  
                results_.value = euroTwoAssetMaxBasketCall(s1_0, s2_0, strike, riskFreeDiscount, 
                    intRate, vol_s1, vol_s2, rho, expiry);

                break;
              // euro put on a two asset max basket 
              case Option::Put:
                results_.value = strike * riskFreeDiscount -  
                    euroTwoAssetMaxBasketCall(s1_0, s2_0, 0, riskFreeDiscount, intRate, 
                        vol_s1, vol_s2, rho, expiry) + 
                    euroTwoAssetMaxBasketCall(s1_0, s2_0, strike, riskFreeDiscount, intRate, 
                        vol_s1, vol_s2, rho, expiry);        
                break;
            }
            break;
          case BasketOption::Min:
            switch (payoff->optionType()) {
              ...
              case Option::Put:
                  results_.value = strike * riskFreeDiscount -                   
                        euroTwoAssetMinBasketCall(s1_0, s2_0, 0, riskFreeDiscount, 
                            intRate, vol_s1, vol_s2, rho, expiry) +         
                        euroTwoAssetMinBasketCall(s1_0, s2_0, strike, riskFreeDiscount, 
                            intRate, vol_s1, vol_s2, rho, expiry);                                       
                break;
            }
            break;
