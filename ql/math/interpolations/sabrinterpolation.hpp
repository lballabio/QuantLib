...
struct SABRSpecs {
    ...
    void defaultValues(std::vector<Real> &params, std::vector<bool> &,
        ...
        if (params[0] == Null<Real>())
            ...
            params[0] = 0.2 * (params[1] < 0.9999 ?
                                   static_cast<Real>(std::pow(forward + (addParams.empty() ? 0.0 : addParams[0]),
                                            1.0 - params[1])) :
                                   1.0);
    ...
    Array inverse(const Array &y, const std::vector<bool> &,
        ...
        Array x(4);
        x[0] = y[0] < 25.0 + eps1() ? static_cast<Real>(std::sqrt(y[0] - eps1()))
                                    : static_cast<Real>((y[0] - eps1() + 25.0) / 10.0);
        // y_[1] = std::tan(M_PI*(x[1] - 0.5))/dilationFactor();
        x[1] = std::sqrt(-std::log(y[1]));
        x[2] = y[2] < 25.0 + eps1() ? static_cast<Real>(std::sqrt(y[2] - eps1()))
                                    : static_cast<Real>((y[2] - eps1() + 25.0) / 10.0);
    ...
    Array direct(const Array &x, const std::vector<bool> &,
        ...
        y[0] = std::fabs(x[0]) < 5.0 ? (x[0] * x[0] + eps1())
                                     : static_cast<Real>((10.0 * std::fabs(x[0]) - 25.0) + eps1());
        // y_[1] = std::atan(dilationFactor_*x[1])/M_PI + 0.5;
        y[1] = std::fabs(x[1]) < std::sqrt(-std::log(eps1()))
                   ? std::exp(-(x[1] * x[1]))
                   : eps1();
        y[2] = std::fabs(x[2]) < 5.0 ? (x[2] * x[2] + eps1())
                                     : static_cast<Real>((10.0 * std::fabs(x[2]) - 25.0) + eps1());
        y[3] = std::fabs(x[3]) < 2.5 * M_PI
                   ? static_cast<Real>(eps2() * std::sin(x[3]))
                   : static_cast<Real>(eps2() * (x[3] > 0.0 ? 1.0 : (-1.0)));
        return y;
    }
