...
// virtually any size you ever might need by the aid of the table of primitive
// polynomials modulo two as provided ready for compilation in the directory
// "PrimitivePolynomialsModuloTwo" on the CD accompanying the book
// "Monte Carlo Methods in Finance" by Peter Jäckel.
// This file and PrimitivePolynomialsModuloTwoUpToDegree27.c are just shortened
// versions of the original files, providing coefficients of primitive
// polynomials modulo two up to degree 18, that is dimensions up to 21200.
//
// If you need higher dimension it is up to you to replace this file and
// PrimitivePolynomialsModuloTwoUpToDegree27.c as distributed with QuantLib
// with the original versions. This will be an easy drop in substitution
// (that's why QuantLib files kept the original names even if they're only
// up to degree 18)
// Then define the macro PPMT_MAX_DIM to a positive
// integer less than or equal to 8129334. That's how many
...
/*! \file PrimitivePolynomialsModuloTwoUpToDegree27.h
    \brief Coefficient of primitive polynomials modulo two
...
// N_PRIMITIVES_UP_TO_DEGREE_xx below.
//
// Example: comment out the line below if you only need dimension 480
//
// #define PPMT_MAX_DIM 480
...
//! Coefficient of primitive polynomials modulo two
/*!
    You can access the coefficient of primitive polynomials modulo two in
    PrimitivePolynomials[i][j] with i and j counting from 0 in C convention.
    PrimitivePolynomials[i][j] will get you the j-th (counting from zero)
    primitive polynomial of degree i+1. Each one-dimensional array of
    primitive polynomials of a given degree is terminated with an entry of -1.
    Accessing beyond this entry will result in a memory violation and must be
    avoided.

    The encoding is as follows: the coefficients of each primitive polynomial
    are the bits of the given integer. The leading and trailing coefficients,
    which are 1 for all of the polynomials, have been omitted.
    Example: The polynomial
        \f[ x^4+x^2+1 \f]
    is encoded as  2  in the array of polynomials of degree 4 because the
    binary sequence of coefficients 10101 becomes 0101 after stripping off
    the top bit, and this is converted to 010 by right-shifting and losing
    the rightmost bit. Similarly, we have
        \f[ x^5+x^4+x^3+x+1 \f]
