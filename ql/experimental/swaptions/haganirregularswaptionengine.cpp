      ...
      volatilityStructure_(std::move(volatilityStructure)) {
        ...
        for (Size i = 0; i < fixedLeg.size(); ++i) {
            ...
            std::shared_ptr<FixedRateCoupon> coupon =
                std::dynamic_pointer_cast<FixedRateCoupon>(fixedLeg[i]);
            QL_REQUIRE(coupon, "dynamic cast of fixed leg coupon failed.");

            expiries_.push_back(coupon->date());

            std::shared_ptr<FixedRateCoupon> newCpn = std::make_shared<FixedRateCoupon>(
            ...
            for (const auto& j : floatLeg) {
                ...
                std::shared_ptr<IborCoupon> coupon = std::dynamic_pointer_cast<IborCoupon>(j);
                QL_REQUIRE(coupon, "dynamic cast of float leg coupon failed.");

                if (coupon->date() <= expiries_[i]) {
                    std::shared_ptr<IborCoupon> newCpn = std::shared_ptr<IborCoupon>(new IborCoupon(
                    ...
                    if (!newCpn->isInArrears())
                        newCpn->setPricer(
                            std::shared_ptr<FloatingRateCouponPricer>(new BlackIborCouponPricer()));
