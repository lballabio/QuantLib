...
namespace QuantLib {
    ...
    class MCPathBasketEngine  : public PathMultiAssetOption::engine,
      ...
      public:
        ...
        typedef typename McSimulation<MultiVariate,RNG,S>::stats_type
                                                                   stats_type;
        // constructor
        MCPathBasketEngine(std::shared_ptr<StochasticProcessArray>,
                           Size timeSteps,
    ...
    class EuropeanPathMultiPathPricer : public PathPricer<MultiPath> {
      public:
        EuropeanPathMultiPathPricer(std::shared_ptr<PathPayoff>& payoff,
      ...
      private:
        std::shared_ptr<PathPayoff> payoff_;
    ...
    template<class RNG, class S>
    inline
    std::shared_ptr<typename MCPathBasketEngine<RNG,S>::path_generator_type>
    MCPathBasketEngine<RNG,S>::pathGenerator() const {

        std::shared_ptr<PathPayoff> payoff = arguments_.payoff;
    ...
    template <class RNG, class S>
    inline
    std::shared_ptr<typename MCPathBasketEngine<RNG,S>::path_pricer_type>
    MCPathBasketEngine<RNG,S>::pathPricer() const {
        ...
        for (Size i = 0; i < numberOfTimes; ++i) {
            ...
            forwardTermStructures[i] = Handle<YieldTermStructure>(
                std::make_shared<ImpliedTermStructure>(riskFreeRate,
                                                         fixings[i]));
        }
    ...
    class MakeMCPathBasketEngine {
      public:
        explicit MakeMCPathBasketEngine(std::shared_ptr<StochasticProcessArray>);
        // named parameters
