...
namespace QuantLib {
    ...
    AtmAdjustedSmileSection::AtmAdjustedSmileSection(const boost::shared_ptr<SmileSection> source, const Real atm, const bool recenterSmile)
        : SmileSection(*source), source_(source) {

        f_ = atm;
        if(f_ == Null<Real>()) f_ = source_->atmLevel();
