...
namespace QuantLib {
    ...
    void MarkovFunctionalCapFloorEngine::calculate() const {
                ...
                if (type == CapFloor::Cap || type == CapFloor::Collar) {
                    ...
                    if(fixingDate <= settlement) {
                        ...
                        for(Size j=0;j<z.size()-1;j++) {
                            price += model_->gaussianShiftedPolynomialIntegral( 0.0, payoff.cCoefficients()[j], payoff.bCoefficients()[j], payoff.aCoefficients()[j], p[j], z[j], z[j], z[j+1] );
                        }
                        ...
                        values[i] = price * model_->numeraire(0.0) * f;
                    }
                }
