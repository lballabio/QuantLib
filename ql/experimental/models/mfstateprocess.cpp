...
namespace QuantLib {
    ...
    Real MfStateProcess::expectation(Time, Real x0,
                                               Time dt) const {
        return x0;
    }

    Real MfStateProcess::stdDeviation(Time t, Real x0,
                                                Time dt) const {
    ...
    Real MfStateProcess::variance(Time t, Real, Time dt) const {
        ...
        for(int k=i;k<j;k++) {
            if(reversionZero_)
                ...
                v+=1.0/(2.0*reversion_)*vols_[k]*vols_[k]*(std::exp(2.0*reversion_*times_[k])-std::exp(2.0*reversion_*std::max(k>0 ? times_[k-1] : 0.0,t)));
        }

        if(reversionZero_)
            v+=vols_[j]*vols_[j]*(t+dt-std::max(j>0?times_[j-1]:0.0,t));
        else
            v+=1.0/(2.0*reversion_)*vols_[j]*vols_[j]*(std::exp(2.0*reversion_*(t+dt))-std::exp(2.0*reversion_*(std::max(j>0?times_[j-1]:0.0,t))));
