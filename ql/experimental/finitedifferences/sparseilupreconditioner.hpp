...
namespace QuantLib {
    ...
    */
	...
	public:
		...
		: L_(compressed_matrix<Real>(A.size1(),A.size2())), 
		  U_(compressed_matrix<Real>(A.size1(),A.size2())) {
			...
			QL_REQUIRE(A.size1() == A.size2(), 
				"SparseILUPreconditioner works only with quadratic matrices");
		...
		const compressed_matrix<Real>& L() const {return L_;};
		const compressed_matrix<Real>& U() const {return U_;};
