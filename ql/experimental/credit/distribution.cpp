...
namespace QuantLib {
    ...
    Distribution::Distribution (Size nBuckets, Real xmin, Real xmax)
    : size_(nBuckets), xmin_(xmin), xmax_(xmax), count_(nBuckets),
      x_(nBuckets,0), dx_(nBuckets,0), density_(nBuckets,0),
      cumulativeDensity_(nBuckets,0), excessProbability_(nBuckets,0),
      cumulativeExcessProbability_(nBuckets,0), average_(nBuckets,0),
      overFlow_(0), underFlow_(0), isNormalized_(false) {
        for (Size i = 0; i < nBuckets; i++) {
            dx_[i] = (xmax - xmin) / nBuckets;
            x_[i] = (i == 0 ? xmin : x_[i-1] + dx_[i-1]);
        }
    }

    Size Distribution::locate (Real x) {
        QL_REQUIRE (x >= x_.front() && x <= x_.back() + dx_.back(),
                    "coordinate out of range [" << x_.front() << "; "
                    << x_.back() + dx_.back() << "]");
    ...
    void Distribution::addDensity (Size bucket, Real value) {
        QL_REQUIRE (bucket >= 0 && bucket < size_, "bucket out of range");
        isNormalized_ = false;
        density_[bucket] += value;
    }

    void Distribution::addAverage (Size bucket, Real value) {
        QL_REQUIRE (bucket >= 0 && bucket < size_, "bucket out of range");
        isNormalized_ = false;
    ...
    void Distribution::normalize () {
        if (isNormalized_ == false) {
            Size count = underFlow_ + overFlow_;
...
//     }
    ...
    Distribution Distribution::convolve (const Distribution& d1,
