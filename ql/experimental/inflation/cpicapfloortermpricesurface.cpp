...
namespace QuantLib {
    ...
    : InflationTermStructure(0,
                             ...
                             dc),
      zii_(zii), observationInterpolation_(observationInterpolation), nominalTS_(std::move(yts)),
      cStrikes_(cStrikes), fStrikes_(fStrikes), cfMaturities_(cfMaturities), cPrice_(cPrice),
      fPrice_(fPrice), nominal_(nominal), bdc_(bdc) {
            ...
            for (Size i = 0; i < cPrice_.rows(); i++) {
                ...
                if (i > 0) {
                    QL_REQUIRE(cPrice_[i][j] <= cPrice_[i - 1][j],
                               "non-decreasing cap prices: " << cPrice_[i][j] << " then "
                                                             << cPrice_[i - 1][j]);
                }
            }
        }
