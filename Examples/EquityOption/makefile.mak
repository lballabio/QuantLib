...
# Primary target:
$(EXE_DIR)\EquityOption-bcb$(_mt)$(_D).exe: $(EXE_DIR) EquityOption.cpp
    if exist $(EXE_DIR)\EquityOption-bcb$(_mt)$(_D).exe \
         del $(EXE_DIR)\EquityOption-bcb$(_mt)$(_D).exe
    bcc32 $(CC_OPTS) -L$(QL_LIB_DIR) \
    -o"EquityOption$(_mt)$(_D).obj" \
    -e"$(EXE_DIR)\EquityOption-bcb$(_mt)$(_D).exe" \
    EquityOption.cpp
