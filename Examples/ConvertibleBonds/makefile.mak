...
# Primary target:
$(EXE_DIR)\ConvertibleBonds-bcb$(_mt)$(_D)-$(VERSION_STRING).exe: $(EXE_DIR) \
                                                          ConvertibleBonds.cpp
    if exist $(EXE_DIR)\ConvertibleBonds-bcb$(_mt)$(_D)-$(VERSION_STRING).exe \
         del $(EXE_DIR)\ConvertibleBonds-bcb$(_mt)$(_D)-$(VERSION_STRING).exe
    bcc32 $(CC_OPTS) -L$(QL_LIB_DIR) \
    -o"ConvertibleBonds$(_mt)$(_D).obj" \
    -e"$(EXE_DIR)\ConvertibleBonds-bcb$(_mt)$(_D)-$(VERSION_STRING).exe" \
    ConvertibleBonds.cpp
