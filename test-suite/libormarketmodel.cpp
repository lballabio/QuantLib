...
namespace libor_market_model_test {
    ...
    std::shared_ptr<IborIndex> makeIndex(std::vector<Date> dates, const std::vector<Rate>& rates) {
    ...
    makeCapVolCurve(const Date& todaysDate) {
        ...
        return std::make_shared<CapletVarianceCurve>(
                         todaysDate, dates,
...
void LiborMarketModelTest::testCalibration() {
    ...
    std::vector<std::shared_ptr<CalibrationHelper> > calibrationHelpers;
    ...
    for (i=2; i < size; ++i) {
        ...
        std::shared_ptr<BlackCalibrationHelper> caphelper(
        ...
        if (i<= size/2) {
            ...
            for (Size j=1; j <= size/2; ++j) {
                ...
                std::shared_ptr<BlackCalibrationHelper> swaptionHelper(
   ...
   // set-up pricing engine
    ...
    for (i=1; i < size; ++i) {
        for (Size j=1; j <= size-i; ++j) {
            ...
            forwardSwap = std::make_shared<VanillaSwap>(
                                Swap::Receiver, 1.0,
                                schedule, swapRate, dayCounter,
                                schedule, index, 0.0, index->dayCounter());
            forwardSwap->setPricingEngine(std::shared_ptr<PricingEngine>(
