...
void CreditRiskPlusTest::testReferenceValues() {
    ...
    if (std::fabs(cr.sectorExposures()[0] - 1000.0) > tol) {
        BOOST_FAIL("failed to reproduce sector 1 exposure (" << cr.sectorExposures()[0]
                                                             << ", should be 1000)");
    }

    if (std::fabs(cr.sectorExposures()[1] - 2000.0) > tol) {
        BOOST_FAIL("failed to reproduce sector 2 exposure (" << cr.sectorExposures()[1]
                                                             << ", should be 2000)");
    }

    if (std::fabs(cr.sectorExpectedLoss()[0] - 40.0) > tol) {
        BOOST_FAIL("failed to reproduce sector 1 expected loss (" << cr.sectorExpectedLoss()[0]
                                                                  << ", should be 40)");
    }

    if (std::fabs(cr.sectorExpectedLoss()[1] - 40.0) > tol) {
        BOOST_FAIL("failed to reproduce sector 2 expected loss (" << cr.sectorExpectedLoss()[1]
                                                                  << ", should be 40)");
    }

    if (std::fabs(cr.sectorUnexpectedLoss()[0] - 30.7) > 0.05) {
        BOOST_FAIL("failed to reproduce sector 1 unexpected loss (" << cr.sectorUnexpectedLoss()[0]
                                                                    << ", should be 30.7)");
    }

    if (std::fabs(cr.sectorUnexpectedLoss()[1] - 31.3) > 0.05) {
        BOOST_FAIL("failed to reproduce sector 2 unexpected loss (" << cr.sectorUnexpectedLoss()[1]
                                                                    << ", should be 31.3)");
    }

    if (std::fabs(cr.exposure() - 3000.0) > tol) {
        BOOST_FAIL("failed to reproduce overall exposure (" << cr.exposure()
                                                            << ", should be 3000)");
    }

    if (std::fabs(cr.expectedLoss() - 80.0) > tol) {
        BOOST_FAIL("failed to reproduce overall expected loss (" << cr.expectedLoss()
                                                                 << ", should be 80)");
    }

    if (std::fabs(cr.unexpectedLoss() - 53.1) > 0.01) {
        BOOST_FAIL("failed to reproduce overall unexpected loss (" << cr.unexpectedLoss()
                                                                   << ", should be 53.1)");
    }
    ...
    if (std::fabs(cr.lossQuantile(0.99) - 250) > 0.5) {
        BOOST_FAIL("failed to reproduce overall 99 percentile (" << cr.lossQuantile(0.99)
                                                                 << ", should be 250)");
    }
}
