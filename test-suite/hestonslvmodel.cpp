...
namespace {
    Real fokkerPlanckPrice1D(const boost::shared_ptr<FdmMesher>& mesher,
                             ...
                             const boost::shared_ptr<StrikedTypePayoff>& payoff, Real x0,
      ...
      const Array::const_iterator upperb = std::upper_bound(x.begin(), x.end(),
              x0);
...
void HestonSLVModelTest::testBlackScholesFokkerPlanckFwdEquation() {
    ...
    const boost::shared_ptr<FdmMesher> uniformMesher(
            new FdmMesherComposite(
                    boost::shared_ptr<Fdm1dMesher>(
                            new FdmBlackScholesMesher(xGrid, process, maturity,
                                    s0))));
    ...
    const boost::shared_ptr<FdmMesher> concentratedMesher(
            new FdmMesherComposite(
                    boost::shared_ptr<Fdm1dMesher>(
                            new FdmBlackScholesMesher(xGrid, process, maturity,
                                    s0, Null<Real>(), Null<Real>(), 0.0001, 1.5,
    ...
    for (Size i = 0; i < LENGTH(strikes); ++i) {
        ...
        const Real calcConcentrated = fokkerPlanckPrice1D(concentratedMesher,
...
void HestonSLVModelTest::testSquareRootZeroFlowBC() {
    ...
    for (Size i = 0; i < 5; ++i) {
        ...
        const Real pm2 = squareRootProcessGreensFct(v_0, kappa, theta, sigma, t,
                vm2);
        const Real pm1 = squareRootProcessGreensFct(v_0, kappa, theta, sigma, t,
                vm1);
        const Real p0 = squareRootProcessGreensFct(v_0, kappa, theta, sigma, t,
                v0);
        const Real p1 = squareRootProcessGreensFct(v_0, kappa, theta, sigma, t,
                v1);
        const Real p2 = squareRootProcessGreensFct(v_0, kappa, theta, sigma, t,
                v2);
        ...
        const Real flowSym4Order = sigma * sigma * v0 / (24 * h)
                * (-p2 + 8 * p1 - 8 * pm1 + pm2)
                + (kappa * (v0 - theta) + sigma * sigma / 2) * p0;
        ...
        const Real fwd2Order = sigma * sigma * v0 / (4 * h)
                * (4 * p1 - 3 * p0 - p2)
                + (kappa * (v0 - theta) + sigma * sigma / 2) * p0;

        const Real fwd3Order = sigma * sigma * v0 / (12 * h)
                * (-p2 + 6 * p1 - 3 * p0 - 2 * pm1)
                + (kappa * (v0 - theta) + sigma * sigma / 2) * p0;
        ...
        if (std::fabs(expected[i][0] - flowSym2Order) > tol
            ...
            BOOST_ERROR(
                    "failed to reproduce Zero Flow BC at" << "\n   v:          " << v << "\n   tolerance:  " << tol);
        }
    }
}
...
void HestonSLVModelTest::testSquareRootEvolveWithStationaryDensity() {
	...
	for (Real sigma = 0.2; sigma < 2.01; sigma += 0.1) {
		const Real alpha = (1.0 - 2 * kappa * theta / (sigma * sigma));
		const Real vMin = invStationaryDistributionFct(kappa, theta, sigma,
				eps);
		const Real vMax = invStationaryDistributionFct(kappa, theta, sigma,
				1 - eps);
		...
		const FdmSquareRootFwdOp::TransformationType transform =
				(sigma < 0.75) ?
						FdmSquareRootFwdOp::Plain : FdmSquareRootFwdOp::Power;
		...
		if (std::fabs(calculated - expected) > tol) {
			BOOST_ERROR(
					"failed to reproduce stationary probability function" << "\n    calculated: " << calculated << "\n    expected:   " << expected << "\n    tolerance:  " << tol);
		}
	}
}
...
void HestonSLVModelTest::testSquareRootLogEvolveWithStationaryDensity() {
	...
	for (Real sigma = 0.2; sigma < 2.01; sigma += 0.1) {
		...
		const Real vMin = std::max(lowerLimit,
				invStationaryDistributionFct(kappa, theta, sigma, eps));
		const Real lowEps = std::max(eps,
				StationaryDistributionFct(kappa, theta, sigma)(lowerLimit));
		...
		critPoints.push_back(
				boost::tuple<Real, Real, bool>(log(vMin), 0.001, false));

		const boost::shared_ptr<FdmMesherComposite> mesher(
				new FdmMesherComposite(
						boost::shared_ptr<Fdm1dMesher>(
								new Uniform1dMesher(log(vMin), log(vMax),
										vGrid))));
		...
		const Real calculated = FdmMesherIntegral(mesher,
				DiscreteSimpsonIntegral()).integrate(p);

		const Real tol = 0.005;
		if (std::fabs(calculated - expected) > tol) {
			BOOST_ERROR(
					"failed to reproduce stationary probability function for " << "\n    sigma:      " << sigma << "\n    calculated: " << calculated << "\n    expected:   " << expected << "\n    tolerance:  " << tol);
		}
	}
}
...
void HestonSLVModelTest::testSquareRootFokkerPlanckFwdEquation() {
	...
	const boost::shared_ptr<FdmMesher> mesher(
			new FdmMesherComposite(
					boost::shared_ptr<Fdm1dMesher>(
							new Uniform1dMesher(lowerBound, upperBound,
									xGrid))));
	...
	for (Size i = 0; i < p.size(); ++i) {
		p[i] = squareRootProcessGreensFct(v0, kappa, theta, sigma, n * dt,
				x[i]);
	}
	...
	for (Size i = 0; i < x.size(); ++i) {
		...
		if (std::fabs(expected - calculated) > tol) {
			BOOST_FAIL(
					"failed to reproduce pdf at" << QL_FIXED << std::setprecision(5) << "\n   x:          " << x[i] << "\n   calculated: " << calculated << "\n   expected:   " << expected << "\n   tolerance:  " << tol);
		}
	}
}
...
void HestonSLVModelTest::testHestonFokkerPlanckFwdEquation() {
	...
	FokkerPlanckFwdTestCase testCases[] = { { 100.0, 0.01, 0.02, 0.05, 1.0,
			0.05, -0.75, std::sqrt(0.2), 101, 401, 25, 25, 0.02, 0.05,
			FdmSquareRootFwdOp::Power, FdmHestonGreensFct::Gaussian,
			FdmSchemeDesc::DouglasType }, { 100.0, 0.01, 0.02, 0.05, 1.0, 0.05,
			-0.75, std::sqrt(0.2), 201, 501, 10, 10, 0.005, 0.02,
			FdmSquareRootFwdOp::Log, FdmHestonGreensFct::Gaussian,
			FdmSchemeDesc::HundsdorferType }, { 100.0, 0.01, 0.02, 0.05, 1.0,
			0.05, -0.75, std::sqrt(0.2), 201, 501, 25, 25, 0.01, 0.03,
			FdmSquareRootFwdOp::Log, FdmHestonGreensFct::ZeroCorrelation,
			FdmSchemeDesc::HundsdorferType }, { 100.0, 0.01, 0.02, 0.05, 1.0,
			0.05, -0.75, std::sqrt(0.05), 401, 501, 5, 5, 0.01, 0.02,
			FdmSquareRootFwdOp::Plain, FdmHestonGreensFct::Gaussian,
...
void HestonSLVModelTest::testHestonFokkerPlanckFwdEquationLogLVLeverage() {
	BOOST_TEST_MESSAGE(
			"Testing Fokker-Planck forward equation "
	...
	const Real upperBound = invStationaryDistributionFct(kappa, theta, sigma,
			0.99);
	const Real lowerBound = invStationaryDistributionFct(kappa, theta, sigma,
			0.01);

	const Real beta = 10.0;
	std::vector<boost::tuple<Real, Real, bool> > critPoints;
	critPoints.push_back(
			boost::tuple<Real, Real, bool>(lowerBound, beta, true));
	critPoints.push_back(boost::tuple<Real, Real, bool>(v0, beta / 100, true));
	critPoints.push_back(
			boost::tuple<Real, Real, bool>(upperBound, beta, true));
	const boost::shared_ptr<Fdm1dMesher> varianceMesher(
	...
	Real denseStrikes[] = { 2.222222222, 11.11111111, 20, 25, 30, 35, 40,
			44.44444444, 50, 55, 60, 65, 70, 75.55555556, 80, 84.44444444,
			88.88888889, 93.33333333, 97.77777778, 100, 102.2222222,
			106.6666667, 111.1111111, 115.5555556, 120, 124.4444444,
			166.6666667, 222.2222222, 444.4444444, 666.6666667 };

	const std::vector<Real> ds(denseStrikes,
			denseStrikes + LENGTH(denseStrikes));
	...
	for (Size strike = 5; strike < 200.0; ++strike) {
		...
		if (std::fabs(expected - calculated) > tol) {
			BOOST_FAIL(
					"failed to reproduce Heston prices at" << "\n   strike      " << strike << QL_FIXED << std::setprecision(5) << "\n   calculated: " << calculated << "\n   expected:   " << expected << "\n   tolerance:  " << tol);
		}
	}
}
...
void lsvCalibrationTest(const FokkerPlanckFwdTestCase& testCase) {
	...
	const FdmSquareRootFwdOp::TransformationType trafoType = testCase.trafoType;
	...
	Array p = FdmHestonGreensFct(mesher,
			boost::shared_ptr<HestonProcess>(
					new HestonProcess(rTS, qTS, spot, v0, kappa, theta, sigma,
							rho)), trafoType, l0).get(eT,
			testCase.greensAlgorithm);
	...
	ModifiedCraigSneydScheme mcg(FdmSchemeDesc::ModifiedCraigSneyd().theta,
			FdmSchemeDesc::ModifiedCraigSneyd().mu, hestonFwdOp);

	DouglasScheme ds(0.5, hestonFwdOp);
	ImplicitEulerScheme ie(hestonFwdOp, ImplicitEulerScheme::bc_set(), 1e-1);
	...
	for (Size i = 1; i < tMesh.size(); ++i) {
		...
		for (Size r = 0; r < 2; ++r) {
			for (Size j = 0; j < x.size(); ++j) {
				...
				const Real vpInt =
						(trafoType == FdmSquareRootFwdOp::Log) ?
...
//    		std::cout << std::endl << std::endl;
	...
	const Real xm = std::log(
			s0 * qTS->discount(maturity) / rTS->discount(maturity))
	...
	for (Size j = 0; j < x.size(); ++j) {
		...
		const Real expected = M_SQRT1_2 * M_1_SQRTPI / stdDev
		...
		if (std::fabs(expected - calculated) > tol) {
			BOOST_FAIL(
					"failed to reproduce probability " << "\n   strike      " << x[j] << QL_FIXED << std::setprecision(5) << "\n   calculated: " << calculated << "\n   expected:   " << expected << "\n   tolerance:  " << tol);
		}
	}
...
//                          << std::endl;
			...
			if (std::fabs((calculated - expected) / vega) > tol) {
				BOOST_FAIL(
						"failed to reproduce round trip vola " << "\n   strike      " << strike << "\n   time        " << times[t] << QL_FIXED << std::setprecision(5) << "\n   calculated: " << localVol + (calculated-expected)/vega << "\n   expected:   " << localVol << "\n   tolerance:  " << tol);
			}
		}
	}
}
...
void HestonSLVModelTest::testLSVCalibration() {
	...
	FokkerPlanckFwdTestCase testCases[] = { { 100.0, 0.035, 0.01, 0.10, 1.0,
			0.1, -0.75, 0.2, 101, 401, 101, 51, 0.00075, 0.00075,
			FdmSquareRootFwdOp::Plain, FdmHestonGreensFct::Gaussian,
			FdmSchemeDesc::ModifiedCraigSneydType }, { 100.0, 0.035, 0.01, 0.12,
			1.0, 0.1, -0.75, 0.2, 101, 401, 101, 51, 0.0005, 0.0005,
			FdmSquareRootFwdOp::Plain, FdmHestonGreensFct::Gaussian,
			...
			100.0, 0.035, 0.01, 0.19, 1.0, 0.1, -0.75, 0.2, 101, 401, 201, 101,
			0.003, 0.003, FdmSquareRootFwdOp::Plain,
			FdmHestonGreensFct::Gaussian, FdmSchemeDesc::HundsdorferType },
// original case, need more time steps for 5 bp roundtrip accuracy
			{ 100.0, 0.035, 0.01, 0.06, 1.0, 0.06, -0.75, std::sqrt(0.2), 201,
					501, 101, 51, 0.0005, 0.0005, FdmSquareRootFwdOp::Log,
