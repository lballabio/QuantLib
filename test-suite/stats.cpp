    ...
    void checkSequence(const std::string& name, Size dimension) {
        ...
        for (i=0; i<dimension; i++) {
            if (calculated[i] != expected) {
                BOOST_FAIL("SequenceStatistics<" << name << ">: " << io::ordinal(i + 1)
                                                 << " dimension: "
                                                 << "wrong minimum value\n"
...
void StatisticsTest::testIncrementalStatistics() {
    ...
    if (stat.samples() != 500000) {
        BOOST_ERROR("stat.samples()  ("
                    << stat.samples() << ") can not be reproduced against cached result (" << 500000
                    << ")");
    }
    ...
    if (std::fabs(stat2.variance() - 1E-2) > tol) {
        BOOST_ERROR("variance (" << stat2.variance() << ") out of expected range " << 1E-2 << " +- "
                                 << tol);
    }
}
