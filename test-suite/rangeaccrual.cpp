...
QL_BEGIN_TEST_LOCALS(RangeAccrualTest)
...
Rate infiniteLowerStrike_, infiniteUpperStrike_;
Real gearing_, correlation_;
Spread spread_;
Date startDate_;
Date endDate_;
Date paymentDate_;
Natural fixingDays_;
DayCounter rangeCouponDayCount_;
boost::shared_ptr<Schedule> observationSchedule_;
// Observation Schedule conventions
Frequency observationsFrequency_;
BusinessDayConvention observationsConvention_;
...
Real flatVol_;
std::vector<boost::shared_ptr<SmileSection> > smilesOnExpiry_;
std::vector<boost::shared_ptr<SmileSection> > smilesOnPayment_; 
...
void createYieldCurve() {
    ...
    termStructure_.linkTo( boost::shared_ptr<YieldTermStructure>(
...
void createVolatilityStructures() {
    ...
    atmVolMatrix_[0][0]=flatVol_; atmVolMatrix_[0][1]=flatVol_; atmVolMatrix_[0][2]=flatVol_; atmVolMatrix_[0][3]=flatVol_;
    atmVolMatrix_[1][0]=flatVol_; atmVolMatrix_[1][1]=flatVol_; atmVolMatrix_[1][2]=flatVol_; atmVolMatrix_[1][3]=flatVol_;
    atmVolMatrix_[2][0]=flatVol_; atmVolMatrix_[2][1]=flatVol_; atmVolMatrix_[2][2]=flatVol_; atmVolMatrix_[2][3]=flatVol_;
    atmVolMatrix_[3][0]=flatVol_; atmVolMatrix_[3][1]=flatVol_; atmVolMatrix_[3][2]=flatVol_; atmVolMatrix_[3][3]=flatVol_;
    atmVolMatrix_[4][0]=flatVol_; atmVolMatrix_[4][1]=flatVol_; atmVolMatrix_[4][2]=flatVol_; atmVolMatrix_[4][3]=flatVol_;
    atmVolMatrix_[5][0]=flatVol_; atmVolMatrix_[5][1]=flatVol_; atmVolMatrix_[5][2]=flatVol_; atmVolMatrix_[5][3]=flatVol_;
...
void createSmileSections() {
    ...
    smilesOnExpiry_ = std::vector<boost::shared_ptr<SmileSection> >();
    smilesOnExpiry_.push_back(boost::shared_ptr<SmileSection>(
        new FlatSmileSection(startDate_, flatVol_, rangeCouponDayCount_)));
    smilesOnExpiry_.push_back(boost::shared_ptr<SmileSection>(
        new InterpolatedSmileSection<>(startDate_, strikes, stdDevsOnExpiry, rangeCouponDayCount_)));
    ...
    smilesOnPayment_.push_back(boost::shared_ptr<SmileSection>(
        new InterpolatedSmileSection<>(endDate_, strikes, stdDevsOnPayment, rangeCouponDayCount_)));
    //smilesOnPayment_.push_back(
    //    swaptionVolatilityStructures_[0]->smileSection(endDate_,
    //                                                   Period(6, Months)));

    QL_REQUIRE(smilesOnExpiry_.size()==smilesOnPayment_.size(),
              "smilesOnExpiry_.size()!=smilesOnPayment_.size()");
}
...
void setup() {
    ...
    gearing_ = 1.0;
    spread_ = 0.0;
    infiniteLowerStrike_ = 1.e-9;
    infiniteUpperStrike_ = 1.0;
    correlation_ = 1.0;

    startDate_ = Date(42800); //6 Mar 2017
    endDate_ = Date(42984);   //6 Sep 2017
    paymentDate_ = endDate_;  //6 Sep 2017
    fixingDays_ = 2;
    rangeCouponDayCount_ = iborIndex_->dayCounter();

    // observations schedule
    observationsConvention_ = ModifiedFollowing;
    observationsFrequency_ = Daily;//Monthly;
    observationSchedule_ = boost::shared_ptr<Schedule>(new 
                                Schedule(startDate_,endDate_,
    ...
    //test parameters
    rateTolerance_ = 2.0e-8;
    priceTolerance_ = 2.0e-4;

}


QL_END_TEST_LOCALS(RangeAccrualTest)
...
void RangeAccrualTest::testInfiniteRange()  {
    ...
    RangeAccrualFloatersCoupon coupon(1.,
                                ...
                                infiniteLowerStrike_,
                                infiniteUpperStrike_);

    Date fixingDate = coupon.fixingDate();

    for (Size z = 0; z < smilesOnPayment_.size(); z++) {
        for (Size i = 0; i < byCallSpread_.size(); i++){
            boost::shared_ptr<RangeAccrualPricer> bgmPricer(new
                RangeAccrualPricerByBgm(correlation_,
                                        smilesOnExpiry_[z],
                                        smilesOnPayment_[z],
                                        true,
                                        byCallSpread_[i]));
                ...
                const Rate indexfixing = iborIndex_->fixing(fixingDate);
    ...
    for (Size z = 0; z < smilesOnPayment_.size(); z++) {
        for (Size i = 0; i < byCallSpread_.size(); i++){
            ...
            for (Size k = 1; k < 100; k++){
                ...
                //Computation
                const Rate price = coupon.price(termStructure_);
