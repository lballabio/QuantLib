...
namespace inflation_cpi_bond_test {
    ...
    std::vector<ext::shared_ptr<Helper> > makeHelpers(const std::vector<Datum>& iiData,
        ...
        for (Datum datum : iiData) {
            ...
            ext::shared_ptr<Helper> h(new ZeroCouponInflationSwapHelper(
                quote, observationLag, maturity, calendar, bdc, dc, ii, yTS));
    ...
    struct CommonVars {
        ...
        CommonVars() {
            ...
            bool interp = false;
            ii = QL_DEPRECATED_SKIP_WARNING(ext::make_shared<UKRPI>(interp, cpiTS));
            ...
            std::vector<ext::shared_ptr<Helper> > helpers =
                makeHelpers(zciisData, ii, observationLag, calendar, convention, dayCounter, yTS);

            Rate baseZeroRate = zciisData[0].rate / 100.0;
            cpiTS.linkTo(
                ext::shared_ptr<ZeroInflationTermStructure>(new PiecewiseZeroInflationCurve<Linear>(
                    evaluationDate, calendar, dayCounter, observationLag, ii->frequency(),
...
void InflationCPIBondTest::testCleanPrice() {
    ...
    CPIBond bond(settlementDays, notional, growthOnly, baseCPI, contractObservationLag, fixedIndex,
                 observationInterpolation, fixedSchedule, fixedRates, fixedDayCount,
                 fixedPaymentConvention);

    ext::shared_ptr<DiscountingBondEngine> engine(new DiscountingBondEngine(common.yTS));
    ext::shared_ptr<InflationCouponPricer> pricer = ext::make_shared<CPICouponPricer>(common.yTS);
