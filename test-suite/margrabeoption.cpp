...
void MargrabeOptionTest::testEuroExchangeTwoAssets() {
    ...
    */
    ...
    for (auto& value : values) {
        ...
        std::vector<std::shared_ptr<StochasticProcess1D> > procs = {stochProcess1, stochProcess2};
...
void MargrabeOptionTest::testGreeks() {
    ...
    for (Real residualTime : residualTimes) {
        ...
        // option to check
        std::shared_ptr<BlackScholesMertonProcess> stochProcess1(new BlackScholesMertonProcess(
            Handle<Quote>(spot1), Handle<YieldTermStructure>(qTS1), Handle<YieldTermStructure>(rTS),
            Handle<BlackVolTermStructure>(volTS1)));

        std::shared_ptr<BlackScholesMertonProcess> stochProcess2(new BlackScholesMertonProcess(
