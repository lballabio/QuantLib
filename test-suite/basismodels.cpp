...
namespace {
	...
	Handle<OptionletVolatilityStructure> getOptionletTS() {
		...
		for (Size i = 0; i < capletVols.size(); ++i) {
			...
			for (Size j = 0; j < capletVols[i].size(); ++j) row.push_back(
				RelinkableHandle<Quote>(ext::shared_ptr<Quote>(new SimpleQuote(capletVols[i][j]))));
		...
		ext::shared_ptr<StrippedOptionletBase>    tmp1(new StrippedOptionlet(2, TARGET(), Following, index, dates, capletStrikes, capletVolQuotes, Actual365Fixed(), Normal, 0.0));
	...
	Handle<SwaptionVolatilityStructure> getSwaptionVTS() {
		...
		ext::shared_ptr<SwaptionVolatilityStructure> tmp(new SwaptionVolatilityMatrix(TARGET(), Following, swaptionVTSTerms, swaptionVTSTerms, swaptionVolQuotes, Actual365Fixed(),true, Normal));
	...
	void testSwaptioncfs(bool contTenorSpread) {
		...
		Schedule fixedSchedule(swapStart, swapEnd, Period(1, Years), TARGET(), ModifiedFollowing, ModifiedFollowing, DateGeneration::Backward, false);
		Schedule floatSchedule(swapStart, swapEnd, Period(6, Months), TARGET(), ModifiedFollowing, ModifiedFollowing, DateGeneration::Backward, false);
		ext::shared_ptr<VanillaSwap> swap(new VanillaSwap(VanillaSwap::Payer, 10000.0, fixedSchedule, 0.03, Thirty360(), floatSchedule, euribor6m, 0.0, euribor6m->dayCounter()));
		...
		for (Size k = 1; k < singleCurveCashFlows.floatWeights().size() - 1; ++k) {
			if (fabs(singleCurveCashFlows.floatWeights()[k]) > tol)
				BOOST_ERROR("Swaption cash flow floating leg spread does not vanish in single-curve setting.\n"
...
void BasismodelsTest::testTenoroptionletvts() {
	...
	Handle<OptionletVolatilityStructure> optionletVTS3m = getOptionletTS();
	{
		...
		ext::shared_ptr<OptionletVolatilityStructure> optionletVTS6m(new TenorOptionletVTS(optionletVTS3m, euribor3m, euribor6m, corr));
		for (Size i = 0; i < capletTerms.size(); ++i) {
			for (Size j = 0; j < capletStrikes.size(); ++j) {
				...
				Real vol6mShifted = optionletVTS6m->volatility(capletTerms[i], capletStrikes[j] + spread, true);
				// De-correlation yields that larger tenor shifted vols are smaller then shorter tenor vols
				if (vol6mShifted - vol3m > 0.0001)  // we leave 1bp tolerance due to simplified spread calculation
					BOOST_ERROR("Shifted 6m vol significantly larger then 3m vol at\n"
						<< "expiry term: " << capletTerms[i] << ", strike: " << capletStrikes[j] << "\n"
...
void BasismodelsTest::testTenorswaptionvts() {
	...
	{
		ext::shared_ptr<TenorSwaptionVTS> euribor3mSwVTS(new TenorSwaptionVTS(euribor6mSwVTS, discYTS, euribor6m, euribor3m, Period(1, Years), Period(1, Years), Thirty360(), Thirty360()));
	...
	{
		ext::shared_ptr<TenorSwaptionVTS> euribor6mSwVTS2(new TenorSwaptionVTS(euribor6mSwVTS, discYTS, euribor6m, euribor6m, Period(1, Years), Period(1, Years), Thirty360(), Thirty360()));
	...
	{
		ext::shared_ptr<TenorSwaptionVTS> euribor3mSwVTS(new TenorSwaptionVTS(euribor6mSwVTS, discYTS, euribor6m, euribor3m, Period(1, Years), Period(1, Years), Thirty360(), Thirty360()));
		ext::shared_ptr<TenorSwaptionVTS> euribor6mSwVTS2(new TenorSwaptionVTS(RelinkableHandle<SwaptionVolatilityStructure>(euribor3mSwVTS), discYTS, euribor3m, euribor6m, Period(1, Years), Period(1, Years), Thirty360(), Thirty360()));
		// 6m vols to 6m vols should yield initiial vols
		for (Size i = 0; i < swaptionVTSTerms.size(); ++i) {
			for (Size j = 0; j < swaptionVTSTerms.size(); ++j) {
				Real vol6m = euribor6mSwVTS->volatility(swaptionVTSTerms[i], swaptionVTSTerms[j], 0.01, true);
